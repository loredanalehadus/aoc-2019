...
namespace AdventOfCode2019Day3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            new ManhattanDistance().Calculate(input);
        }
    ...
    {
        public (int closest, int lowest) Calculate(string[] input)
        {
            var wire1 = input[0].Split(",");
            var wire2 = input[1].Split(",");

            var path1 = new List<string>();
            var path1c = new List<int>();
            ...
            foreach (var item in wire1)
            {
                ...
                var steps = int.Parse(item.Substring(1));
                ...
                for (var i = steps; i > 0; i--)
                {
                    ...
                    if (!path1.Contains($"{xAxys},{yAxys}"))
                    {
                        path1.Add($"{xAxys},{yAxys}");
                        path1c.Add(counter);
                    }
                }
            }
            ...
            setList.AddRange(path1);

            yAxys = 0;
            xAxys = 0;
            counter = 0;

            foreach (var item in wire2)
            {
                var direction = item.Substring(0, 1);
                var steps = int.Parse(item.Substring(1));
                ...
                for (var i = steps; i > 0; i--)
                {
                    ...
                    if (path1.Contains($"{xAxys},{yAxys}"))
                    {
                        var path1position = setList.IndexOf($"{xAxys},{yAxys}");
                        var path1count = path1c[path1position];

                        var sumSteps = path1count + counter;
                        crosses.Add($"{xAxys},{yAxys},{sumSteps}");
                    }
                }
            }
            ...
            {
                var breakup = cross.Split(",");
                var crossXAxys = int.Parse(breakup[0]);
                var crossYAxys = int.Parse(breakup[1]);
                var crossCounter = int.Parse(breakup[2]);

                var distance = Math.Abs(crossXAxys) + Math.Abs(crossYAxys);
